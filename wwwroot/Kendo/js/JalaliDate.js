function JalaliDate() {
    ...
    this.toDateString = function () {
        return this.jalalidate[0].pad() + "/" + (this.jalalidate[1] + 1).pad() + "/" + this.jalalidate[2].pad();
    }
    ...
    else if (arguments.length == 1 && typeof (arguments[0]) === "string" && arguments[0].split('/').length == 3) {
        this.gregoriandate = JalaliDate.parse(arguments[0]).gregoriandate;
    }
    else if (arguments.length == 1 && arguments[0] instanceof JalaliDate) {
...
JalaliDate.parse = function (datestring) {
    try {
        ...
        if (datestring.indexOf("Date(") > -1) {
            ...
            var date = new  JalaliDate(y, m - 1, d); 
            if (datestring.substring(11, 13)) {
                var h = parseInt(datestring.substring(11, 13));
                var m = parseInt(datestring.substring(14, 16));
                date.setHours(h, m, 0);
            }
            return date;
        }
